...
export function createSolanaAdapter(resolved: ResolvedNetwork): ChainAdapter {
  const connection = new Connection(resolved.rpcUrl)
  const facilitatorKeypair = Keypair.fromSecretKey(
    Buffer.from(process.env.FACILITATOR_PRIVATE_KEY_SOLANA!, 'base64'),
  )
  const usdcMint = new PublicKey(resolved.usdc)
