...
export const sellers = pgTable('sellers', {
  ...
  index('idx_sellers_merchant').on(table.merchantId),
])
...
export const mppSessions = pgTable('mpp_sessions', {
  id: uuid('id').primaryKey().defaultRandom(),
  sellerId: uuid('seller_id').references(() => sellers.id),
  buyerAddress: varchar('buyer_address', { length: 255 }).notNull(),
  buyerNetwork: varchar('buyer_network', { length: 50 }).notNull(),
...
export const mppVouchers = pgTable('mpp_vouchers', {
  id: uuid('id').primaryKey().defaultRandom(),
  ...
  index('idx_mpp_vouchers_session').on(table.sessionId),
])
